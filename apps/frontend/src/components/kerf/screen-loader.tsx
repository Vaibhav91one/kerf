// The one screen-level loader — Next's `loading.tsx` convention shows this
// while a route segment's content is being prepared, sidebar/layout untouched
// (see (app)/loading.tsx). The animation is baked at a fixed white-on-dark
// palette (its own <style> block, not var()), same house rule as the
// illustration set (CLAUDE.md's Artwork invariant) — so unlike Illustration,
// which reads fine on either theme's background, this one needs a constant
// dark tile behind it or the dots vanish against a light-mode canvas.

export function ScreenLoader() {
  return (
    <div className="flex min-h-[420px] flex-col items-center justify-center">
      <div className="flex size-[96px] items-center justify-center rounded-[20px] bg-[#121712]">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 56 56" width={56} height={56} role="img" aria-label="Loading">
          <title>Loading</title>
          ...
          <style>{`
            .l{fill:#ffffff;opacity:0;animation:icon-12-k 2200ms cubic-bezier(0.16, 1, 0.3, 1) infinite both;}
            @keyframes icon-12-k{0%{opacity:0;}10%{opacity:1;}55%{opacity:0.85;}100%{opacity:0;}}
